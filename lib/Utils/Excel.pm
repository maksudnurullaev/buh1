...
=cut
    ...
    sub warehouse_prepare {
        ...
        my $objects =
          $scope eq 'current'
          ? Utils::Warehouse::current_list_objects($self)
    ...
    sub get_objects_headers {
        ...
        for my $pid ( keys %{$objects} ) {
            ...
            $counting_field_id = $objects->{$pid}{counting_field}
              if exists $objects->{$pid}{counting_field};
            my $tags = Utils::Db::cdb_get_links(
                $self, $pid,
            ...
            for my $tagid ( keys %{$tags} ) {
                    ...
                    && $tags->{$tagid}{value} )
                {
                    ...
                    {
                        $objects->{$pid}{tags}{ $tags->{$tagid}{name} } =
                          $tags->{$tagid}{value} * (-1);
                    }
                    else {
                        $objects->{$pid}{tags}{ $tags->{$tagid}{name} } =
    ...
    sub get_new_file_path_name {
        ...
        my $file_name = "buga_uz_export_$uuid.xls";
    ...
    sub warehouse_export {
        ...
        return (undef)
          if !scalar( keys( %{$headers} ) ) || !scalar( keys( %{$objects} ) );
        ...
        # Add worksheet
        my $worksheet =
          $workbook->add_worksheet( $self->ml('Warehouse') . ' - buga.uz' );

        my $ord_headers = {};
        my $ord_int     = 1;
        for my $name ( sort { Utils::utf_compare( $self, $a, $b ) }
            keys %{$headers} )
        {
        ...
        for my $pid ( reverse sort keys %{$objects} ) {
            $ord_col = 0;
            $worksheet->write( ++$ord_row, $ord_col++,
                $objects->{$pid}->{description} );
            for my $idx ( sort { $a <=> $b } keys %{$ord_headers} ) {
                my $name = $ord_headers->{$idx};
                if ( exists $objects->{$pid}->{tags}->{$name} ) {
                    $worksheet->write( $ord_row, $ord_col++,
                        $objects->{$pid}->{tags}->{$name} );
                }
                else { $ord_col++; }
            }
        }
        ...
        return ( ( $file_path, $file_name, $objects, $headers ) )
          if $type eq '.xls';
        ( $file_path, $file_name ) =
          make_zipped_file( ( $file_path, $file_name, $type ) );
        return ( ( $file_path, $file_name, $objects, $headers ) );
    }
    ...
    sub merge_parent_and_childs {
        my ( $parent, $childs ) = @_;
        my $fields = [
            'description',     'counting_field',
            'counting_parent', 'counting_direction'
        ];
        my $pid    = $parent->{id};
        my $result = {};
        $result->{$pid} = {};
        for my $field ( @{$fields} ) {
            $result->{$pid}{$field} = $parent->{$field}
              if exists $parent->{$field};
        }
        for my $cid ( keys %{$childs} ) {
            $result->{$cid} = {};
            for my $field ( @{$fields} ) {
                $result->{$cid}{$field} = $childs->{$cid}{$field}
                  if exists $childs->{$cid}{$field};
            }
        }
        return ($result);
    }
    ...
    sub warehouse_export_remains {
        ...
        for my $pid ( @{$pids} ) {
            $ord_row =
              make_remains_outline( $self, $pid, $worksheet, $ord_row, $bold )
              if $pid;
        }
    ...
    sub make_remains_outline {
        ...
        my ( $objects, $headers ) =
          ( merge_parent_and_childs( $parent, $childs ), undef );
    ...
    sub tbalance_export {
        ...
        # Add worksheet
        my $worksheet = $workbook->add_worksheet('Export - buga.uz');
        $worksheet->set_column( 'A:A', 30 );
        ...
        # header
        $ord_row =
          make_tbalance_header( $self, $worksheet, $ord_row, $center, $merge );

        # body
        my $rows = $tbalance->{rows};
        for my $key ( sort keys %{$rows} ) {
            $ord_row =
              make_tbalance_outline( $self, $key, $worksheet, $rows->{$key},
                $tdata, $ord_row, $bold )
              if $key ne 'totals';
        }
        ...
        # footer
        # final
        make_tbalance_footer( $self, $worksheet, $tbalance->{totals}, $ord_row,
            $bold )
          if exists $tbalance->{totals};
        $workbook->close();
        return ( ( $file_path, $file_name ) ) if $self->param('type') eq '.xls';
        ( $file_path, $file_name ) =
          make_zipped_file( ( $file_path, $file_name, $self->param('type') ) );
        return ( ( $file_path, $file_name ) );
    }
    ...
    sub make_tbalance_header {
        ...
        $ord_row ||= 1;
        $worksheet->merge_range( "B$ord_row:C$ord_row",
            $self->param('ml_BALANCE FOR START PERIOD'), $merge );
        $worksheet->merge_range( "D$ord_row:E$ord_row",
            $self->param('ml_BALANCE FOR PERIOD'), $merge );
        $worksheet->merge_range( "F$ord_row:G$ord_row",
            $self->param('ml_BALANCE FOR END PERIOD'), $merge );
    ...
    sub make_tbalance_outline {
        ...
        for my $key (
            qw/start_debet start_credit debet credit end_debet end_credit/)
        {
            if ( exists $row->{$key} ) {
