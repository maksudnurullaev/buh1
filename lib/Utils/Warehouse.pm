...
=cut
    ...
    sub form2data {
        ...
        for my $extra_field (@extra_fields) {
            $data->{$extra_field} = $self->param($extra_field)
              if $self->param($extra_field);
        }
    ...
    sub get_remains_ids_all {
        my $self = shift;
        my $sql =
            " SELECT DISTINCT id, value desription "
    ...
    sub get_remains_ids {
        ...
        my ( $page, $pages, $pagesize ) =
          Utils::Filter::setup_pages( $self, scalar( @{$ids} ) );
    ...
    sub deploy_remains_all {
        ...
        $self->stash( remains_objects => $remains_objects )
          if scalar( @{$rids} );
        return ($rids);
    }
    ...
    sub update_counting_field {
        ...
        if ( !$counting_field_value ) {
            $self->redirect_to(
                "/warehouse/edit/$pid?error=1&error_counting_field=error");
            return;
        }
    ...
    sub clone_object {
        ...
        if ($links) {
            for my $tagid ( keys %{$links} ) {
                if ( $links->{$tagid} eq tag_object_name() ) {
                    my ( $clone_tag, $clone_tag_links ) =
                      get_clear_db_object( $self, $tagid );
        ...
        if ($@) {
                ...
                && exists( $object_clone->{counting_field} ) )
            {
                ...
                set_field( $self, $new_pid, 'counting_field',
                    $new_tags_clone->{$couting_tag_id} );
    ...
    sub get_clear_db_object {
        ...
        return ( $clear_db_object,
            ( exists( $object->{_link_} ) ? $object->{_link_} : undef ) );
    }
    ...
    sub calculated_counting {
        ...
        return (undef)
          if !$parent || !exists( $parent->{counting_field_value} );
        my $pid_cfv = $parent->{counting_field_value};

        my $childs = get_childs_and_counting_field_value( $self, object_name(),
            'counting_parent', $pid );
        my $result;
    ...
    sub get_counting_parent_id {
        my ( $self, $id ) = @_;
        return (undef) if !$self || !$id;
        my $objects = Utils::Db::cdb_get_objects( $self,
            { id => [$id], field => ['counting_parent'] } );
        return ( $objects->{$id}->{counting_parent} )
          if exists( $objects->{$id} )
          && exists( $objects->{$id}->{counting_parent} );
        return $id;
    }
    ...
    sub get_parent_and_counting_field_value {
        ...
        my $db      = Utils::Db::client($self);
        my $objects = $db->get_objects(
            { id => [$pid], field => [ 'description', 'counting_field' ] } );
        if ( $objects && exists( $objects->{$pid} ) ) {
            my $result = $objects->{$pid};
            $result->{counting_field_value} =
              get_linked_field_value( $self, $pid, 'counting_field', 'value' );
            my $tagid       = $result->{counting_field};
            my $tag_objects = $db->get_objects(
                { id => [$tagid], field => [ 'name', 'value' ] } );
    ...
    sub get_childs_and_counting_field_value {
        ...
        return (undef)
          if !$self || !$object_name || !$field_name || !$field_value;
        my $sql =
"SELECT DISTINCT id FROM objects where name = '$object_name' AND field = '$field_name' AND value = '$field_value' ; ";
        my $db  = Utils::Db::client($self);
        my $sth = $db->get_from_sql($sql);
        my $cid = undef;
        $sth->bind_col( 1, \$cid );
        ...
        while ( $sth->fetch ) {
            my $childs = $db->get_objects(
                {
                    id    => [$cid],
                    field => [
                        'description',        'counting_field',
                        'counting_direction', 'counting_parent'
                    ]
                }
            );
            if (   $childs
            ...
            {
                $childs->{$cid}->{counting_field_value} =
                  get_linked_field_value( $self, $cid, 'counting_field',
                    'value' );
    ...
    sub get_childs_counting_eval_str {
        ...
        for my $cid ( keys %{$childs} ) {
            $result .= "$childs->{$cid}->{counting_direction}"
              . $childs->{$cid}->{counting_field_value};
        }
        return ($result);
    }
    ...
    sub get_linked_field_value {
        ...
        return (undef)
          if !$self || !$o1_id || !$o1_o2_field_name || !$o2_field_name;
        my $o1_object =
          Utils::Db::cdb_get_objects( $self,
            { id => [$o1_id], field => [$o1_o2_field_name] } );
        return (undef)
          if !exists( $o1_object->{$o1_id} )
